...
gulp.task('clean', () =>
  del(['dist/', 'dist-es5-module/', 'coverage/', 'mochawesome/'])
);
...
gulp.task('server', () =>
  connect.server({ root: './', port: process.env.PORT || 3000 })
);

gulp.task('test:run', test);
gulp.task('test:watcher', () =>
  gulp.watch(['index.js', 'src/**', 'test/**'], ['test:run'])
);
